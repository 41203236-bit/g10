...
async function startBattle(){
  if (els.startBtn.disabled || !currentRoomCode || currentSlot !== (roomData?.hostSlot || 'O')) return;
  const snap = await get(roomRef(currentRoomCode));
  const fresh = snap.val();
  if (!fresh?.players?.O?.joined || !fresh?.players?.X?.joined) {
    setStatus('雙方都必須加入房間。');
    return;
  }
  const required = ['role','faction','clientId','name'];
  const missingO = required.some(k => !fresh.players.O?.[k]);
  const missingX = required.some(k => !fresh.players.X?.[k]);
  if (missingO || missingX) {
    setStatus('玩家資料尚未同步完成，請雙方重新確認角色與陣營後再開始。');
    showToast('玩家資料未同步完成');
    return;
  }
  const now = Date.now();
  const battle = {
    playerConfig: {
      O: {
        role: fresh.players.O.role,
        faction: fresh.players.O.faction,
        clientId: fresh.players.O.clientId,
        name: fresh.players.O.name
      },
      X: {
        role: fresh.players.X.role,
        faction: fresh.players.X.faction,
        clientId: fresh.players.X.clientId,
        name: fresh.players.X.name
      }
    },
    turn: fresh.hostSlot || 'O',
